// modules/responses/non-stream-handler.ts — Responses 非流分支。
//
// 红线不变量：
//  - aggregator 聚合 + readLoop（idleTimeoutFor(lastCcEvent,false) +
//    readWithTimeout tag 'STREAM_IDLE_TIMEOUT' 不可改，catch 侧靠字符串匹配分流）。
//  - 流前/读中 abort → 499 无 body；超时 429（input_tokens 取请求估算值）；
//    泛错 502 统一不带 retry_after。
//  - 零判定三元组：fullText / reasoningContent / toolCalls.length 任一即非零
//    （对称流式 sawContent）。
//  - 成功收尾：recordTimeoutSuccess + buildResponsesObject（Responses 对象）。
//  - 错误尾（upstreamError / zeroOutput）经 responses/terminal 组装。
...
import { buildResponsesObject, createResponsesAggregator, rawUsageFromCcUsage } from './aggregator'
import { proxy502Response, timeout429Response } from './errors'
import { assembleResponsesTerminal } from './terminal'

export interface ResponsesNonStreamDeps {
  ccResponse: Response
  apiKey: string
...
export async function handleResponsesNonStream(deps: ResponsesNonStreamDeps): Promise<Response> {
  ...
  const aggregator = createResponsesAggregator({
  ...
  if (aggregate.upstreamError || !(
    ...
    return assembleResponsesTerminal(
      {
  ...
  log('info', 'Responses non-stream finish', {
    path: '/v1/responses',
    model,
    responseId,
    streaming: false,
  ...
  return sendJSON(200, buildResponsesObject(model, responseId, createdAt, aggregate))
}
