...
| 78-97 | `callUpstream<T>(args)` | 异步函数 | E | `ensureInitialized` → `forwardToCC` → 非 2xx 时读错误文本、截前 200 字、记录 model 与 apiKey 后 4 位、`log('error', label, ...)` 并返回 `{ok:false,value:onCcError(mapCcError(status,text))}`；2xx 返回 `{ok:true,response}` |
